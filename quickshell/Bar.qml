...
ShellRoot {
    ...
    Process {
        id: battProc
        command: ["sh", "-c", "c=$(cat /sys/class/power_supply/BAT*/capacity 2>/dev/null | head -n1); s=$(cat /sys/class/power_supply/BAT*/status 2>/dev/null | head -n1); echo \"${c:-0}|${s:-Unknown}\""]
        stdout: SplitParser {
            onRead: data => {
                var parts = data.trim().split("|")
                if (parts.length === 2) {
                    var pct = parseInt(parts[0])
                    if (!isNaN(pct)) root.battPct = pct
                    root.battCharging = (parts[1] === "Charging" || parts[1] === "Full")
                }
            }
        }
        Component.onCompleted: running = true
    }
    Timer { interval: 5000; running: true; repeat: true; onTriggered: battProc.running = true }
    ...
    Variants {
        ...
        ActionBar {
            ...
            battPct:      root.battPct
            battCharging: root.battCharging
            wsData:       root.wsData
